...
# -----------
link.may50 <- data.table(link.may50)
############# CHANGE THIS ACCORDING TO THE NEW DATA ##########################################
link.may50[event==0, ':='(a_salida=2017, m_salida=12)]      
...
link.may50[!is.na(EdadInicioDisca44), .N, .(as.numeric(EdadInicioDisca44)>=50)] # 5046 cases before age 50
link.may50[!is.na(EdadInicioDisca13), .N, .(as.numeric(EdadInicioDisca13)>=50)] # 2229 cases
link.may50[is.na(EdadInicioDisca13), .N, .(as.numeric(EdadInicioDisca44)>=50)] #  1604 cases   (we might be able to recover)

# Different way to look at cases
class(link.may50$EdadInicioDisca13) # = factor
...
link.may50 <- link.may50 %>% mutate(CoMo_1 = ifelse(!is.na(K_3_19) & K_3_19=="Sí", 1, 0)) %>% 
  ...
  # 2. Count ADLs
  mutate(CoMo_count = rowSums(.[584:602])) %>% 
...
link.may50 %>% dplyr::count(DISCA13_AGE>=50 & DISCA13_AGE<999) # we have 8632 cases left

### HERE Change entry age by disability variable

link.may50 <- link.may50 %>% dplyr::filter(DISCA13_AGE>=50) %>% dplyr::filter(DISCA13_AGE<110)


# listwise deletion of central variables
link.may50 <- link.may50 %>% dplyr::filter(!is.na(Accident12)) %>% dplyr::filter(!is.na(DailyAct))

###### Central disability variables #######
###### ---------------------------- #######
...
table(link.may50$dis_golpe, useNA = "always")

# golpdata <- as.data.frame(link.may50[,c(6,8,612:625)])
...
# link.may50$DIFF_2 <- apply(link.may50[,626:638], 1, FUN = function(x) {min(x[x > 0])})

link.may50 <- link.may50 %>% mutate(diff1 = ifelse(is.na(diff1), 999, diff1)) %>% mutate(diff2 = ifelse(is.na(diff2), 999, diff2)) %>% 
...
link.may50$DIFF_2 <- apply(link.may50[,624:636], 1, FUN = function(x) {min(x[x > 0])})
link.may50 <- link.may50 %>% mutate(DIFF_2 = as.numeric(ifelse(DIFF_2==999, 0, DIFF_2)))

summary(link.may50$DIFF_2)
